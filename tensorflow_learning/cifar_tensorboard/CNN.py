...
@os : win10
"""
from tensorflow_learning.cifar_test import cifar10
from tensorflow_learning.cifar_test import input

import tensorflow as tf
import numpy as np
import time
from tensorflow_learning.cifar_tensorboard import tools


max_steps = 3000 # 训练轮数
batch_size = 128  #一个bacth的大小
data_dir = './cifar-10-batches-bin' #读取数据文件夹
LOG_DIR = './LOG' # tensorboard可视化的相关文件记录
...
#得到测试集的images和labels
images_test, labels_test = input.inputs(eval_data=True, data_dir=data_dir, batch_size=batch_size)
...
#第一个最大池化层和LRN层(Local Response Normalization，即局部响应归一化层)
with tf.name_scope('pool_norm1'):
    ...
    with tf.name_scope('LRN1'):
        #LRN层可以使模型更加
        """
        提出了LRN层，对局部神经元的活动创建竞争机制，使得其中响应比较大的值变得相对更大，
